...
import { ShieldCheck, LogOut, RefreshCw, Eye, ExternalLink } from 'lucide-react';
...
export const AdminBar: React.FC<AdminBarProps> = ({ onViewPublic }) => {
  ...
  const handleSyncData = async () => {
    try {
      ...
      showToast('기본 포트폴리오 데이터가 Firestore에 성공적으로 동기화되었습니다!', 'success', '동기화 완료');
    } catch (err: any) {
  ...
  const handleLogout = async () => {
    await logout();
    showToast('관리자 계정에서 로그아웃되었습니다.', 'info', '로그아웃');
    if (onViewPublic) onViewPublic();
  };

  return (
    <div className="w-full bg-[#040914] border-b border-cyan-500/30 px-3 sm:px-6 py-2.5 text-xs font-mono shadow-[0_4px_20px_rgba(0,0,0,0.6)]">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-emerald-400 bg-emerald-950/80 px-3 py-1 rounded-full border border-emerald-500/40 shadow-[0_0_12px_rgba(16,185,129,0.2)]">
            ...
            <span className="font-bold">관리자 편집 모드 활성화</span>
          ...
          <span className="hidden lg:inline text-slate-400 text-[11px]">
            • 모든 카테고리(소개, 여정, 수상, 스킬, 프로젝트)의 추가·수정·삭제 가능 (실시간 반영)
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            title="기본 포트폴리오 데이터 전체 동기화"
          >
            <RefreshCw className="w-3.5 h-3.5 text-cyan-400" />
            <span className="hidden sm:inline">기본 데이터 복원</span>
          </button>
